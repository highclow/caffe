...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  const int crop_h = crop_size ? crop_size : param_.crop_h();
  const int crop_w = crop_size ? crop_size : param_.crop_w();
  ...
  CHECK((!crop_h && !crop_w) || (crop_h && crop_w))
      << "Should specify crop_h and crop_w at the same time!";
  ...
  for (int c = 0; c < datum_channels; ++c) {
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        const int offset = (c * height + h) * width;
        top_index = offset + (do_mirror ? width - 1 - w : w );
        if (has_uint8) {
          datum_element =
